...
import { SpotifyCardProps } from '../../types/types';
    ...
    return (
        ...
        <div className="bg-gray-900 shadow-lg rounded p-3">
            <div className="group relative">
                {images?.map((img: any, key) =>
                    key == 0 ?
                        <img className="w-full md:w-72 block rounded" src={img.url} alt="" /> : null

                )

                }
                {artists?.map((art: any, key) =>
                    key == 0 ?
                        <a
            ...
            <div className="p-5">
                <h3 className="text-white text-lg">{nameOfSong}</h3>
                {artists?.map((art: any, key) =>
                    key == 0 ?
                        <p className="text-gray-400">{art.name}</p> : null
